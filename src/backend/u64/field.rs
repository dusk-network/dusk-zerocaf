...
/// 
/// Returns a tuple with both sqrt results: `(x, p-x)`.
impl<'a> ModSqrt for &'a FieldElement {
    type Output = Option<(FieldElement, FieldElement)>;

    fn mod_sqrt(self) -> Option<(FieldElement, FieldElement)> {
        // If the input is `0` the sqrt is directly 0.
        if bool::from(self.ct_eq(&FieldElement::zero())) 
            {return Some((FieldElement::zero(), FieldElement::zero()));}
...
pub mod tests {
    ...
    fn mod_sqrt_tonelli_shanks() {
        ...
        let sqrt = inp.mod_sqrt().unwrap();

        assert!(sqrt.0 == SQRT1_27_NEG);
        assert!(sqrt.1 == SQRT1_27_POS);

        // Test for `0`
        let sqrt_zero = FieldElement::zero().mod_sqrt().unwrap();
        assert!(sqrt_zero.0 == FieldElement::zero());
        assert!(sqrt_zero.1 == FieldElement::zero()); 
    }
