...
use corretto::backend::u64::{field, scalar};
...
mod scalar_benches {
    ...
    pub static B: scalar::Scalar = scalar::Scalar([
        2766226127823335,
        4237835465749098,
        4503599626623787,
        4503599627370493,
    ...
    pub static BA: scalar::Scalar = scalar::Scalar([
        2766226127823335,
        4237835465749098,
        4503599626623787,
        4503599627370491,
    ...
    pub fn bench_mul_internal(c: &mut Criterion) {
        c.bench(
            "mul_internal",
            Benchmark::new("Function", |b| {
                b.iter(|| scalar::Scalar::mul_internal(&B, &BA))
            })
            .with_function("Macro", |b| {
                b.iter(|| scalar::Scalar::mul_internal_macros(&B, &BA))
            }),
        );
    }
}
...
mod field_benches {
    ...
    pub fn bench_modular_inverse(c: &mut Criterion) {
        c.bench(
            "Modular Inverse",
            Benchmark::new("Kalinski inverse", |b| {
                b.iter(|| field::FieldElement::kalinski_inverse(&B))
            })
            .with_function("Savas & Koç inverse", |b| {
                b.iter(|| field::FieldElement::savas_koc_inverse(&B))
            }),
        );
    }
}
    ...
    pub fn bench_point_addition(c: &mut Criterion) {
        c.bench(
            ...
            Benchmark::new("2008 Hisil–Wong–Carter–Dawson, Section 3.1.", |b| {
                b.iter(|| &EdwardsPoint::identity() + &EdwardsPoint::identity())
            }),
        );
    }
}
...
criterion_group!(
    benchmarks,
    scalar_benches::bench_mul_internal,
